...
[![Maven Central](https://img.shields.io/maven-central/v/io.github.codedogapp/gridlink-core.svg?label=Maven%20Central)](https://central.sonatype.com/artifact/io.github.codedogapp/gridlink-core)
...
ag-grid's `sortModel` / `filterModel` JSON binds into typed Java records, and one call turns a request into a
ready-to-run `CriteriaQuery`:
...
Available on [Maven Central](https://central.sonatype.com/artifact/io.github.codedogapp/gridlink-elasticsearch). Add the
adapter &mdash; it pulls in `gridlink-core`; your app already provides `spring-data-elasticsearch`:
...
Using the framework-agnostic model only (no Elasticsearch)? Depend on `gridlink-core` instead.

## Quickstart

**1. Describe your grid's filterable columns.** One typed component per column; `filters()` maps each to its target
document field:
...
That is the whole integration. Filtering, compound `AND`/`OR`, date ranges, multi-column filters, sorting and paging all
fall out of the bound request.
...
- **API reference (Javadoc)** &mdash; [`gridlink-core`](https://javadoc.io/doc/io.github.codedogapp/gridlink-core) · [
  `gridlink-elasticsearch`](https://javadoc.io/doc/io.github.codedogapp/gridlink-elasticsearch)
- **Runnable example** &mdash; [`examples/opensearch-aggrid-demo`](examples/opensearch-aggrid-demo) (ag-grid + Spring
  Boot 4 + OpenSearch)
